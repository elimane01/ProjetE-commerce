<x-app-layout>
    <div class="max-w-xl mx-auto py-10">
        <h1 class="text-2xl font-bold mb-6 text-white">Ajouter un produit</h1>
        <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded shadow text-gray-700 space-y-5">
            @csrf
            <div>
            ...
            <div class="flex gap-4">
                <div class="flex-1">
                    ...
                    <input type="number" step="0.01" name="price" class="w-full border-gray-300 rounded px-3 py-2" value="{{ old('price') }}" required>
                    @error('price') <div class="text-red-500 text-sm">{{ $message }}</div> @enderror
                </div>
                <div class="flex-1">
                    <label class="block mb-1 font-semibold">Stock</label>
                    <input type="number" name="stock" class="w-full border-gray-300 rounded px-3 py-2" value="{{ old('stock', 0) }}" required>
